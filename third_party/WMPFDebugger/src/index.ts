 ...
 * This file remains licensed under GPL-2.0-only; see ../../LICENSE.
 */
import { promises, readFileSync } from "node:fs";
let connectedMiniapps = 0;
import { EventEmitter } from "node:events";
import path from "node:path";
const bridgeToken = JSON.parse(readFileSync(path.join(process.env.WECHAT_DATA_ROOT!, "bridge-token.json"), "utf8")).token;
...
const debugServer = (options: CliOptions, logger: Logger): WebSocketServer  => {
    ...
    wss.on("connection", (ws: WebSocket) => {
        ...
        ws.on("close", () => {
            connectedMiniapps=Math.max(0,connectedMiniapps-1);
...
const proxyServer = (options: CliOptions, logger: Logger): WebSocketServer => {
    const wss = new WebSocketServer({ host: "127.0.0.1", port: options.cdpPort, verifyClient: (info: {req: import("node:http").IncomingMessage}) => !info.req.headers.origin && new URL(info.req.url || "/", "http://127.0.0.1").searchParams.get("token") === bridgeToken });
    logger.info(
    ...
    wss.on("connection", (ws: WebSocket) => {
        ...
        ws.on("message", raw => {
            try { const m=JSON.parse(raw.toString()); if(m.method==="LocalBridge.status"){ws.send(JSON.stringify({id:m.id,result:{connected:connectedMiniapps>0,clients:connectedMiniapps}}));return;} } catch { return; }
