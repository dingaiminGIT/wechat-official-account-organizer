...
- 隔离测试覆盖两类列表的合并与去重、服务号类型在清单和任务中的保留，以及既有身份、白名单、队列、恢复关注保护。实机只读验证通过：正式实现返回 2 个订阅号和 103 个服务号，共 105 个唯一账号，分类统计一致、无无效记录；任选一个服务号的关注状态查询也返回了与清单 ID 一致的布尔状态。验证期间没有发送取关或关注请求。
