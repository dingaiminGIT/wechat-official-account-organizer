...
## 订阅号与服务号统一列表（2026-09-12）

- 微信 4.1.13 的当前 `SubscriptionProfile/personal.html` 使用 `H5ExtTransfer` 命令 27493、路径 `/cgi-bin/mmbiz-bin/bizattr/getusercontactlisth5`，通过 `bizuserattr_servicetype` 和 `get_all:true` 读取公众号通讯录。页面自身使用类型 `0` 读取订阅号；2026-09-12 的本机只读实测确认类型 `1` 返回服务号，因此本版本同时请求类型 `0` 与 `1`，分别标记为订阅号和服务号，并按稳定账号 ID 去重。
- 取关仍使用页面已有的 `Unsubscribe({userName})`。账号状态查询、白名单、账号与会话绑定、清单二次确认、三种执行模式及恢复关注均基于稳定账号 ID，因此两类账号共用相同保护链路。
- 针对微信 4.1.13 / WMPF build 269602 的 Apple Silicon framework，已从 `AppletIndexContainer::OnLoadStart`、scene 1101、`ws://localhost:9421` 与 `SendToClientFilter` 调用链独立定位调试偏移；配置只对精确版本和 SHA-256 指纹启用。
- 隔离测试覆盖两类列表的合并与去重、服务号类型在清单和任务中的保留，以及既有身份、白名单、队列、恢复关注保护。实机只读验证先确认当前关注列表能独立返回订阅号与 103 个服务号，分类统计一致、无无效记录；任选一个服务号的关注状态查询也返回了与清单 ID 一致的布尔状态。最终用户验收页面合并现有取关记录后共显示 339 个账号（订阅号 236、服务号 103），并以超级快速模式完成 66/66 个服务号请求，耗时 59.8 秒。该模式以微信成功回执为准，结果标为「已取关，未复核」。

早期状态（2026-09-07）：已通过当时登录微信的 CDP 页面会话调用订阅号接口，读到 236 条账号记录。全选、持久化白名单、清单预览和串行取关队列已接入；真实取关已验证：用户选择的北京森马、比尔盖茨均收到 UnSubscribe:ok，随后查询 IsSubscribed=false。
