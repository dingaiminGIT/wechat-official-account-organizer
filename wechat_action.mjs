...
try{
 ...
 if(!['health','list','check','unfollow','unfollow_request','follow','follow_request'].includes(input.action))throw Error('无效操作');
 ...
 if(input.action==='health'){console.log(JSON.stringify(health));}
  ...
  if(input.action==='list'){
   ...
   if(['unfollow_request','follow_request'].includes(input.action)){
    const restoring=input.action==='follow_request';
    if(restoring){if(!catalog.accounts.some(a=>a.id===input.id&&(a.was_unfollowed||a.subscribed===false)))throw Error('只能恢复本机取关记录中的账号')}
    else await allowed();
    await identityGuard();stage=restoring?'follow':'unfollow';mutationSent=true;
    const method=restoring?'quicklyAddBrandContact':'Unsubscribe',params=restoring?{username:input.id,webtype:'1',scene:'241'}:{userName:input.id};
    ...
    await identityGuard();console.log(JSON.stringify({id:input.id,status:restoring?'followed_unverified':'unfollowed_unverified',subscribed:restoring,verified:false,mutation_sent:true,ack:ack.ack}));
   }else{
