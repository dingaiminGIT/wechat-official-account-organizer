...
summary: 微信没有批量取关，我就做了一个能分别整理订阅号和服务号、保护白名单并选择三种执行速度的 Mac 工具。
...
[▶ 播放 6 秒真实执行演示（燃向配乐版）](https://img.baiyalab.com/i/2026/09/wechat-organizer-demo-9cf3e08e.mp4)
...
v0.5.0 继续补上了服务号。最终验收时，应用把本机已有记录合并成 339 个账号：订阅号 236 个、服务号 103 个。服务号可以单独筛选，我又用超级快速模式处理了 **66 个服务号，耗时 59.8 秒**；微信返回了全部请求的成功回执，界面仍明确标为「已取关，未复核」。
...
v0.5.0 当前只支持 Apple Silicon，并已验证 macOS 26.6.2、微信 4.1.13 与 WMPF build 269602。因为项目包含 GPLv2 的 WMPFDebugger，整个仓库也按 GPLv2 开源；如果分发修改版，需要继续提供对应源码并保留许可证。当前版本还没有经过 Apple 签名和公证。第一次打开时，请在 Finder 的“应用程序”文件夹里右键点击「批量取关公众号.app」，然后选择“打开”。
